...
reqClient.on('message', function (response) {
  var responseObject = JSON.parse(response.toString());
  var uuid = responseObject.uuid;

  if (responseObject.operation === 'ack_set') {
    subscriptionStrings.forEach(function (topic) {
      subscriber.subscribe(topic + uuid);});} else 

  if (responseObject.operation === 'ack_del') {
